					...
					for($n=0;$n<count($tbl_numeroBAT)-1;$n++)
					{
						...
						$batBaseNameWoNum.=$tbl_numeroBAT[$n] . "_";
							...
							for($b=1;$b<=20;$b++)
							{
								...
								//
								 echo "<hr/><b>" . $batRealDirectory . $batRealName . "</b><br/>";
								 
								if (file_exists($batRealDirectory . $batRealName))
								{
									echo "ok : " . $batRealDirectory . $batRealName;
								}
								else
								{
									echo "ko : " . $batRealDirectory . $batRealName;
								}
								echo "<hr/>";
								if (file_exists($batRealDirectory . $batRealName))
								{
									...
									// On vérifie si il est déjà validé 
											...
											$sqlB="select * from suivi_bats where bat_name like '%" . $batBaseNameWoNum . "%' and state=1 and prefix_contractor='" . "diorhc" . "' order by horodatage desc limit 1";
											...
											if(trim($fullLinkImage)>"")
											{
												...
												echo "<b>Déjà validé</b> <i>" . $tbl_name_image[count($tbl_name_image)-1] . " impossible de passer en attente.</i><br/>";
											}
											else
											{
