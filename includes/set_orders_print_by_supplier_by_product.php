<?php
	...
	require_once("./is_session_active.php");
		...
		$sql.= "id_supplier_produce_by_self=\"" . $_SESSION['id'] . "\",";
		$sql.= "horodatage_supplier_produce_by_self=\"" . date('Y-m-d H:i:s') . "\",";
		if($_SESSION['prefix_contractor']=="mmg" || $_SESSION['prefix_contractor']=="lan")
		{
			$sql.= "status=\"" . 8 . "\" where `type_label`='"  . mysqli_real_escape_string($con,$product) . "' and `num_of`='" . mysqli_real_escape_string($con,$num_of) . "' and `reference`='" . mysqli_real_escape_string($con,$reference) . "'"; // Passe en produced by supplier

		}
		else{
			$sql.= "status=\"" . 8 . "\" where `type_label`='"  . mysqli_real_escape_string($con,$product) . "' and `num_of`='" . mysqli_real_escape_string($con,$num_of) . "' "; // Passe en produced by supplier
