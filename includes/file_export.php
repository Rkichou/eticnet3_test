...
if($_SESSION['prefix_contractor']=='All'){
	echo "<span style='color: red;'>Please select your contractor</span>";	
	exit();
}
else{
	...
	$sql="select * from " . $_SESSION['prefix_contractor'] . "_orders WHERE DATE(date_integration) >= '".$dateFrom."' AND DATE(date_integration) <= '".$dateTo."' and status in (".$statut.") order by num_of asc,type_label desc, id desc";
			...
			switch($value)
			{
				...
				case "code_supplier":
					...
					if(mysqli_num_rows($retourS)>1)
					{
						...
						$lineHead.="SUPPLIER;";
						$chaine=str_replace(";","",$row["$company_name"]); // Elimine les ;
						$chaine=str_replace("\r","_",$chaine); // Elimine les retour chariots
						$chaine=str_replace("\r","_",$chaine); // Elimine les linefeed
					}
					else{
				...
				case "id_printshop":
					$lineHead.="PRINTSHOP;";
					$sqlPrint = "SELECT name FROM printshop WHERE id = ".$row['id_printshop']." ORDER BY id DESC";
					$retourPrint=mysqli_query($con,$sqlPrint);
					$rowPrint=mysqli_fetch_array($retourPrint);
					$chaine=str_replace(";","",$rowPrint['name']); // Elimine les ;
					$chaine=str_replace("\r","_",$chaine); // Elimine les retour chariots
					$chaine=str_replace("\r","_",$chaine); // Elimine les linefeed
					$chaine.=";";
					$ligne.=$chaine;
    ...
    echo "<a href='$fileDownload' download='$fileDownload' class='expbutton'>Export File</a>";
